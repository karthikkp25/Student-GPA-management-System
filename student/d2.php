...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Check if enrollment_id is provided
    if (isset($_POST["enrollment_id"]) && !empty($_POST["enrollment_id"])) {
        $enrollmentID = $_POST["enrollment_id"];

        // SQL to delete the record based on enrollment_id
        $deleteSQL = "DELETE FROM Enrollments WHERE enrollment_id = $enrollmentID";

        // Execute the query
        if ($conn->query($deleteSQL) === TRUE) {
            echo "Record with enrollment_id $enrollmentID has been deleted successfully.";
        } else {
...
<body>
    <header>
        <h1>Delete Enrollment Record</h1>
        <div class="top-right">
            <a href="dele.php" class="btn">menu</a>
        </div>
    </header>

    <div class="container">
        <form method="post" action="">
            <label for="enrollment_id">Enter Enrollment ID to delete:</label>
