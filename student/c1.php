...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["EnrollmentID"]) && !empty($_POST["EnrollmentID"])) {
        ...
        // SQL to delete the record based on enrollment ID
        $deleteSQL = "DELETE FROM Enrollments WHERE enrollment_id = $enrollmentID";

        // Execute the query
        if ($conn->query($deleteSQL) === TRUE) {
            echo "Record with Enrollment ID $enrollmentID has been deleted successfully.";
        } else {
...
<body>
    <header>
        ...
        <div class="top-right">
            <a href="s.php" class="btn">Enrollment Page</a>
        </div>
    </header>

    <div class="container">
        <form method="post" action="">
            <label for="EnrollmentID">Enter Enrollment ID to delete:</label>
